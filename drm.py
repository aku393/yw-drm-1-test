...
class MPDLeechBot:
    ...
    async def split_file(self, input_file, max_size_mb=1900, progress_cb=None, cancel_event=None):
        """Split large files into chunks using size-based splitting only for consistent results"""
        ...
        # Use only size-based splitting for consistency across all formats
        for i in range(num_chunks):
            ...
            # Calculate size-based splitting for consistent results
            ...
            # Use FFmpeg with size limit for precise control
            cmd = [
                'ffmpeg', '-i', input_file,
                '-fs', str(target_size),
                '-c', 'copy',
            ...
            logging.info(f"Creating Part {i+1}/{num_chunks} with target size {format_size(target_size)}")
                ...
                if process.returncode == 0 and os.path.exists(output_file):
                    ...
                    if actual_size > 0:
                        ...
                        if required_parts <= max_parts:
                            ...
                            # Remove the oversized file and try with smaller chunks
                            if os.path.exists(output_file):
                                os.remove(output_file)
                            # Try splitting the entire file with smaller target size
                            smaller_max_mb = max(500, (safe_max_size // (1024 * 1024)) // 2)  # Minimum 500MB
                            logging.info(f"Retrying with smaller chunk size: {smaller_max_mb}MB")
                            return await self.split_file(input_file, smaller_max_mb, progress_cb, cancel_event)
                    else:
                    ...
                    # If FFmpeg fails, try alternative approach with dd command for raw splitting
                    try:
                        logging.info(f"Trying alternative splitting method for part {i+1}")
                        # Use dd to extract exact byte range
                        dd_cmd = [
                            'dd', f'if={input_file}', f'of={output_file}',
                            f'bs=1048576',  # 1MB blocks
                            f'skip={start_byte // 1048576}',
                            f'count={target_size // 1048576}'
                        ]
                        ...
                        await dd_process.communicate()
                        
                        if dd_process.returncode == 0 and os.path.exists(output_file):
                            actual_size = os.path.getsize(output_file)
                            if actual_size > 0:
                                required_parts = (actual_size + part_size - 1) // part_size
                                if required_parts <= max_parts:
                                    chunks.append(output_file)
                                    logging.info(f"✅ Part {i+1}/{num_chunks} (alternative method): {format_size(actual_size)} ({required_parts} Telegram parts)")
                                else:
                                    logging.error(f"❌ Alternative method also produced oversized chunk")
                                    if os.path.exists(output_file):
                                        os.remove(output_file)
                    except Exception as dd_error:
                        logging.error(f"Alternative splitting method failed: {dd_error}")

            except Exception as e:
                logging.error(f"❌ Exception splitting part {i+1}/{num_chunks}: {str(e)}")

        if not chunks:
            raise Exception("Failed to create any valid chunks - check video file integrity and available disk space")
    ...
    async def upload_file(self, event, filepath, status_msg, total_size, sender, duration):
        try:
            ...
            # Set size limits based on admin session capabilities
            # Admin session with string session has premium limits (3.8GB)
            if SESSION_STRING and SESSION_STRING.strip():
                max_size_bytes = min(3600 * 1024 * 1024, telegram_max_file_size - (200 * 1024 * 1024))  # 3.6GB with 200MB safety margin
                user_type = "ADMIN_SESSION"
            else:
                # Bot token has free limits (1.8GB)  
                max_size_bytes = min(1600 * 1024 * 1024, telegram_max_file_size - (300 * 1024 * 1024))  # 1.6GB with 300MB safety margin
                user_type = "BOT_TOKEN"
